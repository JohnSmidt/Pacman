...
function Blinky(x, z, width, height, depth) {
	...
	var ghostCapsuleMaterial2 = new THREE.MeshBasicMaterial( { color:0xFF0000, transparent: true, opacity: 0 } );
...
if(valid) {
	this.direction = blinky.seek(movement, this.positionX, this.positionY);
	//console.log(this.direction);
...
Blinky.prototype.seek = function(movement, x, y) {
	...
	if (this.time <= 0) { 
		...
		switch(this.direction){
			case 1:
				...
				blinkyCapsule.rotation.z -= (8 * Math.PI/180);
				break;
			case 4:
