...
function Pinky(x, z, width, height, depth) {
...
Pinky.prototype.setInternal = function(x, y, z) {
	this.internalX = x;
	this.internalY = y;
	this.internalZ = z;
}
...
Pinky.prototype.getMesh = function() {
	return this.mesh;
}

// Called by game loop on keyboard input.
Pinky.prototype.setDirection = function(direction) {
	this.interrupt = false;
...
Pinky.prototype.tweener = function(clear) {
...
// zero.
Pinky.prototype.interruptTweener = function() {
	if (this.tween > 0) {
		this.tween -= this.tweenSpeed;
		return this.tween;
	} else {
...
Pinky.prototype.validMove = function(direction) {
	switch (direction) {
		case 1:
...
Pinky.prototype.smoothMove = function() {
	var newPosition;
...
Pinky.prototype.ghostAI = function() {
	var valid = true;
	var movement = [];
...
if(valid) {
	var proximity = Math.sqrt(Math.pow(test2 - this.positionX, 2) + Math.pow(test1 - this.positionY, 2));
		if (proximity < 10) {
		 pinky.beDumb(movement)
		}
		else {
	pinky.seek(movement, this.positionX, this.positionY);
	}
	// //console.log(this.direction);
...
Pinky.prototype.beDumb = function(movement) {
...
Pinky.prototype.seek = function(movement, x, y) {
	...
	if (this.time <= 0) { 
		...
		switch(this.direction){
			case 1:
				...
				pinkyCapsule.rotation.z -= (8 * Math.PI/180);
				break;
			case 4:
				this.positionY = this.positionY - 1;
				//pinky.setDirection("z-");
