...
function Inky(x, z, width, height, depth) {
...
Inky.prototype.setInternal = function(x, y, z) {
	this.internalX = x;
	this.internalY = y;
	this.internalZ = z;
}
...
Inky.prototype.getMesh = function() {
	return this.mesh;
}

// Called by game loop on keyboard input.
Inky.prototype.setDirection = function(direction) {
	this.interrupt = false;
...
Inky.prototype.tweener = function(clear) {
...
// zero.
Inky.prototype.interruptTweener = function() {
	if (this.tween > 0) {
		this.tween -= this.tweenSpeed;
		return this.tween;
	} else {
...
Inky.prototype.validMove = function(direction) {
	switch (direction) {
		case 1:
...
Inky.prototype.smoothMove = function() {
	var newPosition;
...
Inky.prototype.ghostAI = function() {
	var valid = true;
	var movement = [];
...
if(valid) {
	var proximity = Math.sqrt(Math.pow(test2 - this.positionX, 2) + Math.pow(test1 - this.positionY, 2));
		if (proximity < 10) {
		 inky.beDumb(movement)
		}
		else {
	inky.seek(movement, this.positionX, this.positionY);
	}
	// //console.log(this.direction);
...
Inky.prototype.beDumb = function(movement) {
...
Inky.prototype.seek = function(movement, x, y) {
	...
	if (this.time <= 0) { 
		...
		switch(this.direction){
			case 1:
				...
				inkyCapsule.rotation.z -= (8 * Math.PI/180);
				break;
			case 4:
